...
if __name__ == '__main__':
    ...
    target = FeatureColumnsExtractor(settings.TARGET).fit_transform(original_dataset).apply(lambda x: np.log(x))
    ...
    output = pd.DataFrame({'Id': original_test_set['Id'],
                           'Hazard': predictions})
    output.to_csv(settings.SUBMIT_RIDGE_LOG, index=False, header=True, columns=['Id', 'Hazard'])
